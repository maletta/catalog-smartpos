...
const { NEXT_PUBLIC_MAIN_API, NEXT_PUBLIC_IMG_API } = process.env;
const storeName = getStoreName();
const pathURL = store => `${NEXT_PUBLIC_MAIN_API}/v1/loja/${store}`;
const baseUrl = url => `${url.protocol}//${url.host}${url.pathname}`;

const getIconPath = (props) => {
  const { codigo, atualizacao } = props;
  return `${NEXT_PUBLIC_IMG_API}store/${codigo}?lastUpdate=${atualizacao}`;
};
